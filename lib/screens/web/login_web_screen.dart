  ...
  Future<void> _handleLogin() async {
    ...
    if (mounted) {
      ...
      if (success) {
        ...
        debugPrint('🔵 Comparaison avec centreAnalyse: ${role == UserRole.centreAnalyse}');
        
        if (role == UserRole.centreAnalyse) {
          debugPrint('✅ Redirection vers CenterDashboardWeb');
          Navigator.of(context).pushAndRemoveUntil(
            MaterialPageRoute(builder: (_) => const CenterDashboardWeb()),
            (route) => false,
          );
        } else {
          debugPrint('❌ Rôle non autorisé: $role');
          _showErrorSnackBar('Accès réservé aux centres d\'analyse. Rôle détecté: ${role?.value ?? "inconnu"}');
        }
      } else {
